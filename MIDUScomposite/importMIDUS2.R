...
newnames = c('M2ID',
  ...
  'like_to_lead','talk_people_into','influencing','decisions','not_afraid',
  'Age','Sex',
...
midus2$not_afraid = 8 - midus2$not_afraid 





### Make exploration confirmation splits

table(midus2$Dominant, useNA = 'ifany')
table(!is.na(midus2$Dominant))
table(!is.na(midus2$like_to_lead))
table(!is.na(midus2$not_afraid))

midus2 <- midus2[!is.na(midus2$Age)&!is.na(midus2$Sex),]

set.seed(1234567)
#sub.midus2 = createDataPartition(c(midus2$Age,midus2$Sex), times=2, p=0.5, list=TRUE)
sub.midus2 = createFolds(c(midus2$Age,midus2$Sex), k=2, list=TRUE, returnTrain = TRUE)
head(sub.midus2[[1]])
length(sub.midus2[[1]])
table(midus2[sub.midus2[[1]],'Dominant'], useNA = 'ifany')
table(midus2[sub.midus2[[2]],'Dominant'], useNA = 'ifany')

mid.e = midus2[sub.midus2[[1]]]
mid.c = midus2[sub.midus2[[2]]]
