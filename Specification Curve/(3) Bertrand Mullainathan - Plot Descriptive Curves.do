	...
	*For main effect
		gen b=eblack
		gen p=pblack
		label var b  "Marginal effect: Black"
	
	*for interaction
		replace b=einter
		replace p=pinter
		label var b  "Marginal effect: Black*Quality interaction"
...
*Specification in paper
	...
	label var b_paper "Original Specification"
...
*Graph parameters
	...
	scatter k1c k2c k3c  sk, xlab(0(10)90) xsize(10) ysize(6) msize(small small small)  ///
	...
	scatter b_ns  sk, yaxis(2) mcolor(gray) msize(small) legend(order(4 5 6) cols(3) size(small) rowgap(.5) pos(12)  )   ytitle("{bf:Black*Quality interaction}",axis(2) placement(north))  || ///
	scatter b_sig sk,yaxis(2) mcolor(black) msize(small)  ylab(, axis(2) labsize(tiny) angle(0) ) yscale(range($from_y $bmax) axis(2))  ///
	yline(0, axis(2) lw(thin) lcolor(gray))  yline(3 24 23 8 22 29, lw(thick) lcolor(white)) || ///
	scatter b_paper sk, yaxis(2)  msize(medium) mcolor(red)  
	


*Save graph in high definition
	*IF I run the main effect run this
		graph export "C:\uri\research\Specification Curve\Tables\Fig1 Main Effect Black - Descriptive.png", width(1400) replace
	*If I run the interaction effect run this
		graph export "C:\uri\research\Specification Curve\Tables\Fig2 Interaction - Descriptive.png", width(1400) replace
