...
*Create the specification curve for the observed data
	tempname scurve
	postfile `scurve' monte k1 k2 k3 k4 k5 k6 k7 b p em ef edif   using monte_hurricanes_199_250 ,replace
	
	
*(0) MONTE LOOP
	set seed 100
	forvalues montek=199(1)250 {
	
	*Shuffle names
		if `montek'>0 {
		shufflevar masfem female name,joint cluster(post79) dropold  //Shuffle jointly "masfem" and "female", within pre-post 1979 period, drop the old variables
		rename masfem_shuffled masfem                                //Give the shuffled variables the same name
		rename female_shuffled female
		rename name_shuffled name
		}
*(1) Outliers loop
	    local sk 1
	    forvalues k1 =1(1)3 {
...
*(7) Year covariates;
   ...
   *RUN `SPEC' AND SAVE RESULTS (use capture so taht if estimation does not converge i move on to the next one
		capture quietly: `spec'
	...
	*SAVE RESULTS
		...
		*Measure fit of model
			...
			*save results
				post `scurve' (`montek')  (`k1') (`k2') (`k3') (`k4') (`k5') (`k6') (`k7') (`b') (`p') (`em') (`ef') (`edif')
