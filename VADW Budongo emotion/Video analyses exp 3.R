...
videos.new <- read.csv("C:/Users/s1229179/Downloads/Data compiled.csv", header = T, na.strings = "NA")
...
dat <- data.frame(ID=11:13, FOO=c('a|b','b|c','x|y'))
foo <- data.frame(do.call('rbind', strsplit(as.character(dat$FOO),'|',fixed=TRUE)))

dat <- within(vid.full, split <- data.frame(
  do.call('rbind', strsplit(as.character(vid.full$Outcome),'.',fixed=TRUE))))
